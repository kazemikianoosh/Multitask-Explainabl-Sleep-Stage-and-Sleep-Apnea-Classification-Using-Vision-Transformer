...
class Block(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        x, attn_scores = self.attn(x, x, training=training, return_attention_scores=True)
        self.attention_output = x  # Store the attention output
    ...
    def get_attention_softmax(self):
        return tf.nn.softmax(self.attention_scores, axis=-1)
    
    def get_layer_norm(self, inputs):
        x = self.norm_before(inputs, training=False)
        x, weights = self.attn(x, x, training=False, return_attention_scores=True)
        x = self.stochastic_depth([inputs, x], training=training)
        x2 = self.norm_after(x, training=training)
        return x2
    
    def get_projection_matrix(self, inputs):
        x = self.norm_before(inputs, training=False)
        self.attn(x, x, training=False, return_attention_scores=True)
        
        weight_names = ['query', 'keys',  'values', 'proj']
        for name, out in zip(weight_names, self.attn.get_weights()):
            if name=='proj':
                return out
...
class VisionTransformer(tf.keras.Model):
    ...
    def get_attention_softmax(self, inputs: tf.Tensor, block_num: int):
        x = self.embeddings(inputs, training=False)
        for i, block in enumerate(self.blocks):
            x = block(x, training=False)
            if i == block_num:
                return block.get_attention_softmax()

    def get_mlp_weights(self, block_num: int):
        return self.blocks[block_num].mlp.dense1.kernel, self.blocks[block_num].mlp.dense2.kernel
    ...
    def get_last_layernorm(self, inputs: tf.Tensor):
        x = self.embeddings(inputs, training=False)
        for block in self.blocks[:-1]:
            x = block(x, training=False)
        return self.blocks[-1].get_layer_norm(x)
    
    
    def get_last_block_attention_output(self, inputs: tf.Tensor):
        x = self.embeddings(inputs, training=False)
        for block in self.blocks:
            x = block(x, training=False)
        return self.blocks[-1].get_attention_output()
    
    def get_last_projection_matrix(self, inputs: tf.Tensor):
        x = self.embeddings(inputs, training=False)
        for block in self.blocks:
            x = block(x, training=False)
        return self.blocks[-1].get_projection_matrix(x)
